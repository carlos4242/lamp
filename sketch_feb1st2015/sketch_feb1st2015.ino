...
void getLatestWeather() {
  ...
  if (interruptCounter > 20) {
    ...
    if (readingWeatherReply) {
      DEBUG_OUT("reset weather connection");
      lineLength = 0;
      finish = true;
    }
  }
...
void readSerialCommands() {
  if (Serial.available() > 0) {
    ...
    if(incomingByte == 97){ // a
      ...
      debug = !debug;
      if (debug) {
        DEBUG_OUT("http debug on");
      } 
      else {
        DEBUG_OUT("http debug off");
      } 
    }
  }
}
...
char * readRequestLine(const char * requestLine) {
  ...
  if (strncmp(requestLine,getLightString,strlen(getLightString)) == 0) {
    ...
    if (debug) {
      ...
      DEBUG_OUT("Invalid http request");
      ...
      if (firstLine) {
        ...
        if (gotHeaders && (output || !postFunction)) {
          if (sendFavicon) {
            ...
            DEBUG_OUT("sent favicon");
          } 
          else if (sendWebsite) {
            writeWebsite(client);
            DEBUG_OUT("sent website");
          }
          else {
...
void sendFaviconToClient(EthernetClient client) {
  ...
  if (faviconInRam) {
    ...
    DEBUG_OUT("failed to get favicon buffer");
    DEBUG_OUT(errno);
  }
}
...
void writeWebsiteSection(EthernetClient client, const char * section, const int length) {
  ...
  if (sectionInRam) {
    ...
    DEBUG_OUT("could not write website section - could not malloc");
  }
}
