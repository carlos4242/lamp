...
void getLatestWeather() {
  ...
  const byte weatherServerAddress[] = { 
    10,0,1,102         };
  ...
  if (interruptCounter > 20) {
    ...
    if (readingWeatherReply) {
      Serial.println("reset weather connection");
      lineLength = 0;
      finish = true;
    }
  }
...
char * changeLightsStatus(const char * postBody) {
  const char * allOnParam = "allOn=1";
  const char * allOffParam = "allOff=1";
...
char * readRequestLine(const char * requestLine) {
  const char * getLightsString = "GET /lights";
  const char * getLightString = "GET /lights/";
  const char * getFavicon = "GET /favicon";
  const char * getWebsite = "GET /";
  const char * postLightsString = "POST /lights";
  const char * postLightString = "POST /lights/";
...
void writeHttpReply(EthernetClient client, const char * output) {
...
void listenForEthernetClients() {
  ...
  if (client) {
    ...
    while (client.connected()) {
      ...
      if (gotHeaders) {
        lineLength = 0;
        while (client.available() && lineLength < lineBufferLen) { // check for buffer overrun
